...
function select_multiple_windows_and_save() {
  ...
  jq -n '{windows: []}' > "$CONFIG" # reset state
  ...
  for index in "${indices[@]}"; do
    if [[ "$index" =~ ^[0-9]+$ ]] && ((index >= 1 && index <= ${#windows[@]})); then
      ...
      jq --arg title "$title" --arg id "$win_id" \
         ...
         '.windows += [{title: $title, id: $id, x: $x, y: $y, width: $w, height: $h}]' "$CONFIG" > "$tmp" && mv "$tmp" "$CONFIG"
...
function restore_single_saved_window() {
  ...
  w=$(jq -r ".windows[$n].width" "$CONFIG")
  h=$(jq -r ".windows[$n].height" "$CONFIG")

  wmctrl -lG | grep "$title" | while read -r line; do
    win_id=$(echo "$line" | awk '{print $1}')
    wmctrl -i -r "$win_id" -e "0,$x,$y,$w,$h"
    echo "✅ Restored \"$title\" to ($x,$y) ${w}x${h}"
  done
}

function loop_restore_all() {
  echo "🔁 Watching for saved windows..."
  while true; do
    count=$(jq '.windows | length' "$CONFIG")
    for ((i=0; i<count; i++)); do
      title=$(jq -r ".windows[$i].title" "$CONFIG")
      ...
      w=$(jq -r ".windows[$i].width" "$CONFIG")
      h=$(jq -r ".windows[$i].height" "$CONFIG")
...
function menu() {
  echo ""
  echo "🔥 Window Position Tracker — Multi-Window + Size Mode"
